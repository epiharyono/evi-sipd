...
function singkron_sp2d_lokal_per_jenis(type_data, status, page=1, response_all=[], cb){
    ...
    setTimeout(() => {
	    relayAjaxApiKey({
	        ...
	        url: config.service_url+'pengeluaran/strict/sp2d/pembuatan/index?status='+status+'&page='+page,
	        type: 'get',
	        success: function (response) {
	            console.log('SP2D', response);

							console.log('singkron data disini bbbb');
...
async function post_sp2d_localdb(response){
		response.map(function(b, i){
				// baru yasmin caca
				setTimeout(() => {
						singkron_sp2d_ke_lokal_skpd(b, 'type_data', 'status', ()=>{
							// resolve_reduce(nextData);
							});


						// singkron_sp2d_cetak_ke_lokal_skpd(b, ()=>{
						// 	// resolve_reduce(nextData);
						// 	});

						console.log('ini post_sp2d_localdb : '+i, b);
				}, 2000 * i)
		})
